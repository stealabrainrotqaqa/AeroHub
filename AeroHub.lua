...
--  muito mais barato que caçar chamada por chamada.
--
--  _G.SXEFreeVerboso = true ANTES de executar devolve o console real,
--  pra quando EU precisar depurar.
-- ============================================================
--  UM local so. Com `local print, warn = print, warn` o arquivo estourou
--  os 200 registradores do Luau no O0. Os warn viram print logo abaixo,
--  entao um slot resolve os dois.
local print = print
if not _G.SXEFreeVerboso then
    local frases = {
        "[AERO] warming up the spaghetti reactor...",
        "[AERO] asking the server politely for permission",
        "[AERO] counting all the pixels, please hold",
        "[AERO] reticulating splines (37% done)",
        "[AERO] the hamster is tired, deploying second hamster",
        "[AERO] downloading more RAM",
        "[AERO] teaching the brainrots to sit",
        "[AERO] bribing the physics engine",
        "[AERO] this log is intentionally useless",
        "[AERO] converting vibes into frames per second",
        "[AERO] sharpening the pointy brackets",
        "[AERO] the cake is a lie, the pets are not",
        "[AERO] rerouting power from life support to cosmetics",
        "[AERO] negotiating with the lag",
        "[AERO] pretending to compute something important",
        "[AERO] if you are reading this, you are early",
        "[AERO] feeding the loop, do not disturb",
        "[AERO] all systems nominal, probably",
        "[AERO] tightening one (1) loose bolt",
        "[AERO] the answer is 42, the question is pending",
    }
    local convites = {
        ">> AERO FREE  --  join discord.gg/heresy for updates",
        ">> get the full version at discord.gg/heresy",
        ">> stuck? support lives at discord.gg/heresy",
        ">> discord.gg/heresy  --  giveaways, updates, help",
        ">> AERO HUB FREE  --  discord.gg/heresy",
        ">> tell your friends: discord.gg/heresy",
    }
    local n = 0
    local function troll()
        n = n + 1
        --  1 convite a cada 4 linhas: presente o bastante pra ser lido,
        --  raro o bastante pra nao virar spam que o usuario ignora.
        if n % 4 == 0 then
    ...
    --  um convite a cada 45s, independente do que o hub esteja fazendo
    task.spawn(function()
        while true do
            task.wait(45)
    ...
    real(">> AERO HUB  FREE EDITION  --  discord.gg/heresy")
end

-- ============================================================
--  BUSCA DE GUI NOSSA  --  precisa vir ANTES de tudo
--
--  As ScreenGuis do hub sairam da PlayerGui (o jogo enumera aquilo e
--  a atualizacao passou a kickar por causa disso) e foram pro gethui().
--  Os FindFirstChild que olhavam so a PlayerGui passaram a devolver
--  nil, e a logica de esconder painel parava de esconder -- todos os
--  paineis apareciam de uma vez.
--
--  ESTE BLOCO MORA NO TOPO DE PROPOSITO: a primeira versao ficava na
--  linha 2483 e era chamada na 1452. `_G.SXEBuscaGui` era nil ali, e a
--  GUI nao nascia.
-- ============================================================
...
-- ============================================================
--  BARRA DO TOPO  --  Aero Hub | FPS/PING | discord
--
...
do
    ...
    -- ============================================================
    --  TITULO EM DUAS PARTES  --  portado da versao paga
    --
    --  La sao "AERO HUB" em degrade vermelho e "PRIVATE" em dourado.
    --  Aqui a segunda palavra e FREE, em AZUL/BRANCO.
    --
    --  O degrade varre com Offset de -1 a 1, entao as duas PONTAS da
    --  ColorSequence tem que ser a MESMA cor -- senao a volta do ciclo
    --  tem emenda visivel. E o claro fica numa banda estreita no meio,
    --  que le como brilho correndo pelas letras. Foi a licao do titulo
    --  vermelho da paga: com as pontas escuras o texto sumia na maior
    --  parte do ciclo.
    -- ============================================================
    local function _tituloGrad(label, cores, seg)
        if not label then return end
        label.TextColor3 = Color3.new(1, 1, 1)   -- o degrade multiplica
        local g = Instance.new("UIGradient")
        g.Name = "AeroTitleGrad"
    ...
    nHub.TextSize = 13
    nHub.Parent = nome
    --  AZUL nas pontas, BRANCO na banda do meio. O branco corre pelas
    --  letras como brilho -- e o oposto do vermelho da paga, onde o
    --  escuro e que passava.
    _tituloGrad(nHub, {
        ColorSequenceKeypoint.new(0.00, Color3.fromRGB( 18,  68, 150)),
        ColorSequenceKeypoint.new(0.30, Color3.fromRGB( 42, 130, 240)),
        ColorSequenceKeypoint.new(0.50, Color3.fromRGB(255, 255, 255)),
        ColorSequenceKeypoint.new(0.70, Color3.fromRGB( 42, 130, 240)),
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB( 18,  68, 150)),
    }, 3.2)

    --  FREE INVERTE: branco nas pontas, AZUL VIVO no meio. Com as duas
    --  palavras no mesmo degrade elas viravam uma mancha so; invertida,
    --  FREE se destaca sem sair da paleta.
    ...
    _tituloGrad(nFree, {
        ...
        ColorSequenceKeypoint.new(0.28, Color3.fromRGB(150, 205, 255)),
        ColorSequenceKeypoint.new(0.50, Color3.fromRGB( 30, 125, 245)),
        ColorSequenceKeypoint.new(0.72, Color3.fromRGB(150, 205, 255)),
    ...
    stats.Text = "FPS: -- | PING: --"
    stats.TextColor3 = Color3.fromRGB( 46,  74, 116)   -- lilas sumia no branco
...
-- ============================================================
-- ============================================================
--  PALETA  --  mesma ESTRUTURA da versao paga, em AZUL/BRANCO
--
--  As luminancias sao as da paga, valor por valor: so a matiz mudou.
--  Isso preserva contraste de texto e hierarquia de painel -- se eu
--  clareasse ou escurecesse "pra ficar azul", quebraria a legibilidade
--  que la ja esta calibrada.
--
--  DE ONDE VINHA O ROXO: o acento antigo era 139,92,246 -- roxo de
--  verdade. Os cinzas frios (azul > vermelho) NAO sao o problema aqui:
--  na paga eles foram esquentados porque o acento e vermelho, e cinza
--  frio brigando com acento quente e o que o olho le como roxo. Com o
--  acento AZUL, cinza frio e coerente -- e o certo.
--
--  Red/Red2 continuam VERMELHOS de proposito: eles sinalizam ESTADO
--  (perigo, blacklist, remover). Se virassem azul, "acento da marca" e
--  "isto vai apagar algo" seriam a mesma cor.
-- ============================================================
Themes = {
    Light = {
...
AERO = { NOME="AERO", GRAD_A=Color3.fromRGB(24,110,225), GRAD_B=Color3.fromRGB(120,195,255) }

Theme = {}
--  Nasce ESCURO, como a paga. O tema claro continua disponivel pelo
--  toggle Dark Mode da GUI.
...
function applyTheme(themeName)
    ...
    local function forceWhiteText(inst)
        if inst:IsA("TextButton") then
            local txt = inst.Text
            if txt == "ON" or txt == "OFF" or txt == "ADD" or txt == "X" or txt == "â–²" or txt == "â–¼" then
...
Keybinds = {
    ...
    ["Ragdoll Self"]="R",Float="Z",["Auto Buy"]="K",
    ["Click to AP"]="NONE",
    ["Steal Nearest"]="NONE",
    ["Face Nearest"]="NONE"
}
...
actionConfig = {
    ...
    ["Kick (Y)"]=true,["Kick To Private"]=true,["JP"]=true,
...
-- Zerado DEPOIS do loadConfig: senao o arquivo salvo reativaria tudo.
do
...
--  Lia o cooldown na GUI de admin do jogo. Sem painel, nada a ler.
...
AP_COMMAND_EMOJIS={balloon="ğŸˆ",inverse="ğŸ”„",jail="ğŸ”’",jumpscare="ğŸ‘»",morph="ğŸ­",nightvision="ğŸŒ™",ragdoll="ğŸŒ€",rocket="ğŸš€",tiny="ğŸœ"}
...
-- ============================================================
--  PAINEL DE ADMIN REMOVIDO
--
--  Ponto UNICO por onde todo comando de admin passava. Nao disparava
--  remote: manipulava a GUI de admin do PROPRIO JOGO (playerGui.
--  AdminPanel), ligava ela e "clicava" nos botoes por firesignal ou
--  VirtualInputManager -- assinatura de executor, nao de jogador.
--
--  Nesta versao o fireClick era usado SO por aqui (2783/2800), ao
--  contrario da versao paga, onde ele tambem serve ao instantClone do
--  TP. La remove-lo mataria o auto steal; aqui ele so fica orfao.
-- ============================================================
...
local function setProximityAP(on)
    ...
    -- Never save ProximityAP to config Ã¢â‚¬â€ always starts OFF
    Config.ProximityAP = false
...
local function ShowPriorityAlertImpl(brainrotName, genText, mutation, ownerUsername)
    ...
    titleLabel.Text = brainrotName .. " â€¢ " .. genText
...
function flyForwardTo(hrp, tpPos, lookDir, targetY, customSpeed)
    ...
    while hrp.Parent do
        ...
        if tpPos.Y <= 10 then
            ...
            -- the target. The old code only rose when a wall was ~20 studs away â€”
            ...
            elseif myPos.Y > targetFloorY + 5 then
                -- Coasting mid-arc with no obstacle ahead â€” hold altitude, no snap
                yVel = 0
            else
                -- Already low and nothing tall ahead â€” snap to target ground so
    ...
    if not g and inp.UserInputType==Enum.UserInputType.MouseButton1 and Config.ClickToAP then
        ...
        if bestPlayer then
            ...
            if runAdminCommand(bestPlayer, picked) then
                local emoji = AP_COMMAND_EMOJIS[picked] or "âš¡"
                ShowNotification("CLICK AP", emoji .. " " .. picked .. " â†’ " .. bestPlayer.DisplayName)
            end
        end
    end
end)
...
function setXRay(enabled)
    ...
    if enabled then
        ...
        task.spawn(function()
            -- AÃˆâ„¢teptÃ„Æ’m sÃ„Æ’ se ÃƒÂ®ncarce folder-ul Plots
            ...
            -- Initial call cu un mic delay sÃ„Æ’ dÃ„Æ’m timp primelor modele sÃ„Æ’ aparÃ„Æ’
            if currentLoopId ~= xrayLoopId then return end
            pcall(applyTransparencyToAllPlotsXRay, alphaPercent, currentLoopId)
            
            -- Loop continuu Ãˆâ„¢i super-optimizat (fÃ„Æ’rÃ„Æ’ overlap de thread-uri)
...
-- we get ~50 attempts/sec, and with a 3-fire burst per tick that's ~150/sec â€”
...
_G.ShowStealProgressBar = function(targetName, duration)
    ...
    title.Text = "âš¡ STEALING: " .. (targetName or "Brainrot"):upper()
...
function makeHeader(f,t,isMain)
    local bar=Instance.new("Frame"); bar.Name="AeroBar"; bar.Size=UDim2.new(1,0,0,2)
    bar.BackgroundColor3=(AERO and AERO.GRAD_A) or Theme.Accent; bar.BorderSizePixel=0; bar.ZIndex=5; bar.Parent=f
    local g=Instance.new("UIGradient"); g.Rotation=0
    g.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,(AERO and AERO.GRAD_A) or Theme.Accent),
                               ColorSequenceKeypoint.new(1,(AERO and AERO.GRAD_B) or Theme.AccentLight)}); g.Parent=bar
    ...
    local function marca(txt)
        ...
        return (txt:gsub("SXE HUB PRIVAT",(AERO and AERO.NOME) or "AERO"))
    end
...
function makeSyncStateRow(parent,text,toggleName,callback,bindName)
    ...
    if bindName then
        local kb=Instance.new("TextButton"); kb.Name="AeroBind"; kb.Size=UDim2.new(0,56,0,22)
...
function makeSyncMainToggle(parent,text,toggleName,callback,bindName)
    ...
    if bindName then
        local kb=Instance.new("TextButton"); kb.Name="AeroBind"; kb.Size=UDim2.new(0,56,0,21)
...
-- mostrava nada. "Aero Hub" tambem vira a chave de posicao/tamanho
...
main,mainBody=makeMainPanel("Aero Hub",UDim2.new(0,375,0,480),UDim2.new(0.5,-187,0.5,-255))
    ...
    local function buildRPButtons()
        ...
        for _, cmd in ipairs(order) do
            if Config.AdminPanelButtons and Config.AdminPanelButtons[cmd] then
                table.insert(btns, {AP_COMMAND_EMOJIS[cmd] or "âš¡", cmd})
            end
        end
        if #btns == 0 then btns = {{"ğŸŒ€","ragdoll"},{"ğŸ”’","jail"},{"ğŸš€","rocket"},{"ğŸˆ","balloon"}} end
        return btns
    end
...
function makePriorityRow(index)
    ...
    local up=Instance.new("TextButton"); up.Name="WhiteTextBtn"; up.Size=UDim2.new(0,26,0,22); up.Position=UDim2.new(1,-86,0.5,-11); up.BackgroundColor3=Theme.Accent; up.Text="â–²"; up.TextColor3=Color3.new(1,1,1); up.Font=Enum.Font.GothamBold; up.TextSize=10; up.Parent=row; corner(up,5)
    local dn=Instance.new("TextButton"); dn.Name="WhiteTextBtn"; dn.Size=UDim2.new(0,26,0,22); dn.Position=UDim2.new(1,-56,0.5,-11); dn.BackgroundColor3=Theme.Accent; dn.Text="â–¼"; dn.TextColor3=Color3.new(1,1,1); dn.Font=Enum.Font.GothamBold; dn.TextSize=10; dn.Parent=row; corner(dn,5)
...
function loadTab(tabName)
    ...
    if tabName=="Keybinds" then
        ...
        for _,name in ipairs({"Kick","Rejoin Job ID","Clone","Manual TP","Invisible Steal","Job ID","Proximity","Carpet Boost","Open Menu","Ragdoll Self","Float","Auto Buy","Click to AP","Steal Nearest","Face Nearest"}) do makeKeybindRow(mainBody,name) end
    ...
    elseif tabName=="Auto TP" then
        ...
        makeMainToggle(mainBody,"Fly TP",Config.TpSettings.FlyTP,function(on)
            ...
            -- Fly TP and Grabble TP are mutually exclusive â€” Fly TP flies over
    ...
    elseif tabName=="UI" then
        -- AERO EXTRAS: guardado porque loadTab pode rodar na inicializacao,
        -- antes do modulo do fim do arquivo existir
        if _G.AeroExtras then
            makeSyncMainToggle(mainBody,"Face Away","Face Away",function(on) _G.AeroExtras.setFaceAway(on) end)
            makeSyncMainToggle(mainBody,"Face Nearest Player","Face Nearest",function(on) _G.AeroExtras.setFaceNearest(on) end,"Face Nearest")
        end
        ...
        -- White Mode / Dark Mode buttons removed Ã¢â‚¬â€ dark mode only
    ...
    elseif tabName=="Misc" then
        if _G.AeroExtras then
            makeSyncMainToggle(mainBody,"Anti Colisao (atravessar players)","Anti Colisao",function(on) _G.AeroExtras.setAntiColisao(on) end)
            makeSyncMainToggle(mainBody,"Anti Flasher (remove acessorios)","Anti Flasher",function(on) _G.AeroExtras.setAntiFlasher(on) end)
            makeSyncMainToggle(mainBody,"Ps On Steal","Ps On Steal",function(on) _G.AeroExtras.setPs(on) end)
        end
        ...
        local function makeAPConfigPanel(panelKey, titleText, configTable, orderKey, onToggleCb)
            ...
            for idx, cmd in ipairs(cmdOrder) do
                local emoji = AP_COMMAND_EMOJIS[cmd] or "âš¡"
...
UIS.InputBegan:Connect(function(input,gp) if gp then return end
    ...
    local actions={
        ...
        ["Face Nearest"]=function()
            if _G.AeroExtras then
                _G.AeroExtras.setFaceNearest(not Config.FaceAwayNearest)
            end
        end,
...
if Config.TpSettings.TpOnLoad then task.spawn(function()
    ...
    --
    --  A ordem antiga era: preparo -> TP. SÃ³ que no instante do
...
local function carpetEngage()
    ...
    local function goToBrainrot(petPos)
        ...
        elseif h >= 11 and h <= 23.15 then
            ...
            -- literally â€” character ends up at petY - 8 (8 studs below the pet)
    ...
    local function doVelocityTP()
        ...
        -- V3 Floor 2 from Floor 1 is intentionally NOT routed here â€” it always goes
...
do
    ...
    local function construirBotaoPs()
        ...
        bar.BackgroundColor3 = (AERO and AERO.GRAD_A) or Theme.Accent
        bar.BorderSizePixel = 0
        bar.ZIndex = 3
        bar.Parent = psBtn
        local g = Instance.new("UIGradient")
        g.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, (AERO and AERO.GRAD_A) or Theme.Accent),
            ColorSequenceKeypoint.new(1, (AERO and AERO.GRAD_B) or Theme.AccentLight),
        })
        g.Parent = bar
        ...
        UIS.InputEnded:Connect(function(i)
            ...
            or i.UserInputType == Enum.UserInputType.Touch then
                if arrastando and moveu then
                    pcall(function() rememberPosition("AERO Ps Button", psBtn) end)
                end
                arrastando = false
            end
        end)
        ...
        click.MouseButton1Click:Connect(function()
            ...
            if _G.AeroExtras then _G.AeroExtras.setPs(not Config.PsOnSteal) end
        end)

        pcall(function() applySavedPosition("AERO Ps Button", psBtn) end)
    ...
    local function marcaGarantir()
        ...
        mkFolder = Instance.new("Folder"); mkFolder.Name = "AeroFaceAway"; mkFolder.Parent = cam
    ...
    local function construirPainel()
        ...
        painel, corpo = makeQuickPanel("AERO\nPanel Face Away",
        ...
        pcall(function() applySavedPosition("AERO\nPanel Face Away", painel) end)
    end
